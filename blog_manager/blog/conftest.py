...
def open_read_and_control_write(monkeypatch):
    ...
    for cls in [blog_views.SiteViewSet, blog_views.CategoryViewSet, blog_views.AuthorViewSet]:
        ...
        class WritePerm(BasePermission):
            def has_permission(self, request, view):
                if request.method in SAFE_METHODS:
                    return True
                u = request.user
                return bool(u and u.is_authenticated and (getattr(u, "is_staff", False) or u.groups.filter(name__iexact="Publisher").exists()))
        WritePerm = WritePerm
